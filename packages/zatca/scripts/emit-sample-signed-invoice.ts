...
import { ZATCA_INVOICE_STATUS } from "../src/modules/zatca/lib/lifecycle";
...
async function main(): Promise<void> {
  ...
  if (
    record.status !== ZATCA_INVOICE_STATUS.PENDING ||
    record.icv === null ||
    !record.pih ||
    !record.invoice_hash ||
    !record.xml
  ) {
    throw new Error("sample invoice did not produce a chained pending document");
  }
