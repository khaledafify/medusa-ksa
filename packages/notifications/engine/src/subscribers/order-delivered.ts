...
/** Handle Medusa delivery creation events with a provider-agnostic SMS notification. */
export default async function orderDeliveredNotificationHandler({
  ...
  await handleOrderNotification({
    ...
    eventName: EVENTS.ORDER_DELIVERED,
    eventData: event.data,
    source: ORDER_NOTIFICATION_SOURCES.ORDER_ID,
  });
}

/** Subscriber registration for order delivery notifications. */
