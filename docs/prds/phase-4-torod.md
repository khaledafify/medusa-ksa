...
- **S3 — Book + label + cancel.** `createFulfillment` (book at fulfillment → tracking number + Torod shipment ref in data). `getFulfillmentDocuments` (on-demand label fetch). `cancelFulfillment` (idempotent; terminal no-op).
