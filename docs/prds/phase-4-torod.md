...
- **S3 — Book + label + cancel.** `createFulfillment` two-step: `POST /order/create` → `order_id`, then `order/ship/process` with `courier_partner_id` → store `tracking_id` + **`aws_label` URL** on fulfillment data; box count from fulfillment-data override else `TOROD_DEFAULT_BOX_COUNT`. `getFulfillmentDocuments` returns the **stored `aws_label` URL** (no separate label endpoint). `cancelFulfillment` → `/shipments/cancel` (ready-for-pickup only; terminal = idempotent no-op).
  *Accept:* mocked unit tests cover booking/label/cancel (the commit gate). **Live sandbox booking is wallet-gated** — Torod's sandbox returns 406 "Please recharge your wallet" until the account is funded, so the live booking lives in a wallet-gated integration test that **skips on the 406 / no creds** (mirrors moyasar `live.integration.test.ts`), and **status stays `🚧 Beta`** until it passes. (Live API confirmed: token field `data.bearer_token`; `warehouse` = the warehouse **code** e.g. `MEDUSAKSASANDBOX`, not the numeric id; `filter_by` ∈ `cheapest|fastest`.)
...
- **S7 — Sandbox e2e + status.** Rate → book → track → (return) against Torod sandbox in `apps/demo-store`; `pnpm changeset`.
  *Accept:* mocked e2e green; the live rate→book→track e2e is **wallet-gated** (skips until the Torod sandbox wallet is funded); status `🚧 Beta` → `✅ Stable` **only** once the live e2e passes against a funded wallet.
